...
export const DateRangePicker = ({ value: valueProp, defaultValue, onChange, onApply, onCancel, ...props }: DateRangePickerProps) => {
    ...
    const formattedStartDate = value?.start instanceof Date ? formatter.format(value?.start) : value?.start ? formatter.format(value.start.toDate(getLocalTimeZone())) : "Select date";
    const formattedEndDate = value?.end instanceof Date ? formatter.format(value?.end) : value?.end ? formatter.format(value.end.toDate(getLocalTimeZone())) : "Select date";
